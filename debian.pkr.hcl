...
EOF
  ...
  provisioner "file" {
    source      = "configure/prometheus_metric_generator.jar"
    destination = "/tmp/prometheus_metric_generator.jar"
  }

  provisioner "file" {
    source      = "configure/pmg_app.service"
    destination = "/tmp/pmg_app.service"
  }
  ...
  provisioner "shell" {
    inline = [
      "sh -cx 'sudo mv /tmp/prometheus_metric_generator.jar /usr/share/prometheus_metric_generator.jar'",
      "sh -cx 'sudo mv /tmp/pmg_app.service /etc/systemd/system/pmg_app.service'"      
    ]
  }
